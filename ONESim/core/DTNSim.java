...
public class DTNSim {
	...
	private static void initSettings(String[] confFiles, int firstIndex) {
		...
		//如果没有confFiles，这个数组里面会被塞入一个null，使得长度为1
		...
		try {
			//第一个配置文件应该是调用init方法
			//如果在没有给出任何配置文件名称的情况下，init方法会读取default settings
	 ...
	 * 这个注册是在各个需要reset的类的static块中进行调用的
